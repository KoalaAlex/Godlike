...
public class NetworkPlayerGodlike : NetworkBehaviour
{
    ...
    void Awake()
    {
		...
		NetworkGameManagerGodlike.sShips.Add(this);
    }
    ...
    void Start()
    {
		...
		if (NetworkGameManagerGodlike.sInstance != null)
    ...
    public void Init()
    {
		...
		scoreGO.transform.SetParent(NetworkGameManagerGodlike.sInstance.uiScoreZone.transform, false);
        _scoreText = scoreGO.AddComponent<Text>();
        _scoreText.alignment = TextAnchor.MiddleCenter;
		_scoreText.font = NetworkGameManagerGodlike.sInstance.uiScoreFont;
    ...
    void OnDestroy()
    {
		NetworkGameManagerGodlike.sShips.Remove(this);
    }
    ...
    public void Kill()
    {
        ...
        if (lifeCount > 0)
        {
			...
			NetworkGameManagerGodlike.sInstance.StartCoroutine(NetworkGameManagerGodlike.sInstance.WaitForRespawn(this));
        }
    }
    ...
    public void CreateBullets()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
			...
			NetworkGodlikeBullet bulletScript = bullet.GetComponent<NetworkGodlikeBullet>();
